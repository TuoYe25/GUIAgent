...
// ---------------------------------------------------------------------------
// Model Registry — single source of truth for all plan models
...
function populateModelSelect() {
  ...
  // Separator + disabled (local self-host) models
  if (entries.some(([, m]) => !m.enabled)) {
    const sep = document.createElement('option');
    sep.disabled = true;
    sep.textContent = '── Local models (self-host) ──';
    $sel.appendChild(sep);
  ...
  // Always add regex parser at the end
  ...
  regexOpt.textContent = 'Regex Parser (offline)';
  $sel.appendChild(regexOpt);
}
...
async function executeWorkflow(prompt) {
  ...
  let actions;
  const modelChoice = state.$modelSelect?.value || 'regex';

  if (modelChoice !== 'regex') {
    const entry = MODEL_REGISTRY[modelChoice];
    if (entry && entry.enabled) {
      actions = await callModel(entry, prompt, elements);
      if (!actions || actions.length === 0) {
        logMessage('warn', 'Model returned no actions, falling back to regex parser');
      }
    } else {
      logMessage('warn', `Model "${modelChoice}" not available, falling back to regex parser`);
    }
  }
...
// ---------------------------------------------------------------------------
// Unified Model Calling — routes to text or vision model based on registry entry
// ---------------------------------------------------------------------------

const TEXT_SYSTEM_PROMPT = `You are a browser automation agent. Your job is to translate a user's natural-language instruction into a sequence of UI actions on a web page.
...
const VISION_SYSTEM_PROMPT = `You are a browser automation agent with vision capabilities. Your job is to translate a user's natural-language instruction into a sequence of UI actions on a web page.
...
function buildElementsSummary(elements) {
...
async function callModel(entry, prompt, elements) {
  if (!entry || !entry.enabled) return null;

  const elementsSummary = buildElementsSummary(elements);
  const typeLabel = entry.type === 'vision' ? 'VLM' : 'LLM';
  const systemPrompt = entry.type === 'vision' ? VISION_SYSTEM_PROMPT : TEXT_SYSTEM_PROMPT;

  let imageBase64 = null;
  if (entry.type === 'vision') {
    logMessage('info', 'Capturing screenshot for vision model...');
    try {
      const screenshotResult = await window.electronAPI?.screenshot();
      if (!screenshotResult?.success || !screenshotResult.image) {
        logMessage('error', 'Failed to capture screenshot');
        return null;
      }
  ...
  if (entry.type === 'vision') {
    ...
    userContent = JSON.stringify({ prompt, elements: elementsSummary });
  }

  const extraInfo = entry.type === 'vision'
    ? `screenshot + ${elementsSummary.length} elements`
    : `${elementsSummary.length} elements`;
  logMessage('info', `Calling ${entry.label} (${extraInfo})...`);
  ...
  try {
    ...
    if (!result?.success) {
      logMessage('error', `${typeLabel} API error: ${result?.error || 'unknown'}`);
      return null;
    }

    const actions = parseModelResponse(result.data);
    if (!actions) {
      logMessage('error', `${typeLabel} returned non-array actions`);
      return null;
    }

    logMessage('success', `${typeLabel} planned ${actions.length} actions`);
    return actions;
  } catch (err) {
    logMessage('error', `${typeLabel} call failed: ${err.message}`);
    return null;
  }
}
